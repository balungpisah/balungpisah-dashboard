...
const INDO_GEO_URL = "https://raw.githubusercontent.com/denyherianto/indonesia-geojson-topojson-maps-with-38-provinces/refs/heads/main/TopoJSON/indonesia-38-provinces.topo.json";
...
export default function IndonesiaMap({
  ...
  const maxHeat = useMemo(() => {
    const counts = provinceHeat.map((p: any) => p.value);
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-xl bg-slate-200 border border-slate-300 ${className}`} style={{ minHeight: '400px' }}>
      
      {/* Zoom Controls UI */}
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
        <button onClick={handleZoomIn} className="p-2 bg-white rounded-md shadow-md hover:bg-slate-50 border border-slate-200 text-slate-600">
          <Plus size={18} />
        </button>
        <button onClick={handleZoomOut} className="p-2 bg-white rounded-md shadow-md hover:bg-slate-50 border border-slate-200 text-slate-600">
          <Minus size={18} />
        </button>
        <button onClick={handleReset} className="p-2 bg-white rounded-md shadow-md hover:bg-slate-50 border border-slate-200 text-slate-600">
          <RotateCcw size={18} />
        </button>
      </div>
      ...
      >
        ...
        >
          <Geographies geography={INDO_GEO_URL}>
            {({ geographies }) =>
              geographies.map((geo) => {
                ...
                const heat = provinceHeat.find((h: any) => 
                  normalizeName(h.label) === normalizeName(geoName)
                );            
                return (
                  <Geography
                    key={geo.rsmKey}
          ...
          {activeMarkers.map((point: any) => (
            <Marker
              key={point.id}
              coordinates={[Number(point.lon || point.lng), Number(point.lat)]}
            >
              <g 
      ...
      <div className="absolute bottom-4 left-4 pointer-events-none">
        {hoveredName ? (
          <div className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-xl animate-in fade-in slide-in-from-bottom-2">
            {hoveredName}
          </div>
        ) : (
          <div className="bg-white/50 backdrop-blur-sm text-slate-500 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-widest font-semibold border border-slate-200">
            Indonesia Map
          </div>
        )}
      </div>
